...
namespace Nez.Tiled
{
...
#endif
	{
		...
		public class CollisionState
		{
			...
			public bool ShouldTestPlatforms = true;
							  ...
							  out int collisionResponse)
		{
			...
			for (var i = 0; i < _collidingTiles.Count; i++)
			{
				...
				if (TestTileCollision(_collidingTiles[i], side, perpindicularPosition, leadingPosition,
					shouldTestSlopes, collisionState.ShouldTestPlatforms, out collisionResponse))
				{
		...
		/// <param name="shouldTestPlatforms">Should test platforms.</param>
		...
		bool TestTileCollision(TmxLayerTile tile, Edge edgeToTest, int perpindicularPosition, int leadingPosition,
							   bool shouldTestSlopes, bool shouldTestPlatforms, out int collisionResponse)
		{
			...
			if (tile.IsOneWayPlatform())
			{
				...
				if (edgeToTest != Edge.Top || shouldTestPlatforms is false)
					return false;
